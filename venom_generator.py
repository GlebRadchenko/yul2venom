...
class VenomIRBuilder:
    ...
    def _handle_intrinsic(self, func, args):
        ...
        if func in ("add", "mul", "not", "eq", "iszero", "and", "or", "xor", "addmod", "mulmod", "signextend", "mload", "sload", "tload", "calldataload", "callvalue", "calldatasize", "codesize", "returndatasize", "gas", "address", "caller", "origin", "gasprice", "chainid", "basefee", "timestamp", "number", "difficulty", "gaslimit", "lt", "gt", "slt", "sgt"):
            return self.current_bb.append_instruction1(func, *args)
    ...
    def _emit_deduplicated(self, op, *operands):
        # Venom backend forbids duplicate consumed operands in a single instruction (e.g. staticcall(..., %1, ..., %1))
        # unless the variable is live afterwards. Since we can't guarantee liveness here without expensive analysis,
        # we must alias duplicate variables.
        ...
        for arg in operands:
            if isinstance(arg, IRVariable):
                if arg.name in seen_vars:
                    # Duplicate usage! Create alias using add(x, 0) to avoid AssignElimination
                    # %alias = add %original, 0
                    alias_var = self.current_bb.append_instruction("add", arg, IRLiteral(0))
                    new_operands.append(alias_var)
                    # Don't add alias to seen_vars (logic allows strict unique variable NAMES)
                    continue
                else:
