...
type IHooks_0 is address;

/// @dev Two `int128` values packed into a single `int256` where the upper 128 bits represent the amount0
/// and the lower 128 bits represent the amount1.
type BalanceDelta_0 is int256;

using BalanceDeltaLibrary_0 for BalanceDelta_0 global;

/// @notice Library for getting the amount0 and amount1 deltas from the BalanceDelta type
library BalanceDeltaLibrary_0 {
    function amount0(BalanceDelta_0 balanceDelta) internal pure returns (int128 _amount0) {
        assembly ("memory-safe") {
            _amount0 := sar(128, balanceDelta)
        }
    }

    function amount1(BalanceDelta_0 balanceDelta) internal pure returns (int128 _amount1) {
        assembly ("memory-safe") {
...
interface IPoolManager {
    ...
    function swap(PoolKey memory key, SwapParams memory params, bytes calldata hookData)
        external
        returns (BalanceDelta_0 swapDelta);
...
library UnsafePoolManager {
    ...
    ) internal returns (BalanceDelta_0 r) {
        assembly ("memory-safe") {
            let ptr := mload(0x40)
...
// src/core/PancakeInfinity.sol

/// @dev Two `int128` values packed into a single `int256` where the upper 128 bits represent the amount0
/// and the lower 128 bits represent the amount1.
type BalanceDelta_1 is int256;

/// @notice Library for getting the amount0 and amount1 deltas from the BalanceDelta type
library BalanceDeltaLibrary_1 {
    /// @notice Constant for a BalanceDelta of zero value
    BalanceDelta_1 public constant ZERO_DELTA = BalanceDelta_1.wrap(0);

    function amount0(BalanceDelta_1 balanceDelta) internal pure returns (int128 _amount0) {
        assembly ("memory-safe") {
            _amount0 := sar(0x80, balanceDelta)
        }
    }

    function amount1(BalanceDelta_1 balanceDelta) internal pure returns (int128 _amount1) {
        assembly ("memory-safe") {
...
using BalanceDeltaLibrary_1 for BalanceDelta_1 global;
...
IPancakeInfinityVault constant VAULT_0 = IPancakeInfinityVault(0xdeaDDeADDEaDdeaDdEAddEADDEAdDeadDEADDEaD); // TODO: replace
...
interface IPancakeInfinityCLPoolManager is IPancakeInfinityPoolManager {
    ...
    /// Additionally note that if interacting with hooks that have the BEFORE_SWAP_RETURNS_DELTA_FLAG or AFTER_SWAP_RETURNS_DELTA_FLAG
    /// the hook may alter the swap input/output. Integrators should perform checks on the returned swapDelta.
    function swap(PoolKey_0 memory key, SwapParams memory params, bytes calldata hookData)
        external
        returns (BalanceDelta_1 delta);
}
...
interface IPancakeInfinityBinPoolManager is IPancakeInfinityPoolManager {
    ...
    function swap(PoolKey_0 memory key, bool swapForY, int128 amountSpecified, bytes calldata hookData)
        external
        returns (BalanceDelta_1 delta);
}
...
abstract contract PancakeInfinity is SettlerAbstract {
    ...
    function sellToPancakeInfinity(
        ...
        bytes memory encodedBuyAmount = _setOperatorAndCall(
            address(VAULT_0), data, uint32(IPancakeInfinityLockCallback.lockAcquired.selector), _pancakeInfinityCallback
        );
...
IBalancerV3Vault constant VAULT_1 = IBalancerV3Vault(0xbA1333333333a1BA1108E8412f11850A5C319bA9);
...
abstract contract BalancerV3 is SettlerAbstract, FreeMemory {
    ...
    function sellToBalancerV3(
        ...
        bytes memory encodedBuyAmount =
            _setOperatorAndCall(address(VAULT_1), data, uint32(uint256(uint160(recipient)) >> 128), _balV3Callback);
        // buyAmount = abi.decode(abi.decode(encodedBuyAmount, (bytes)), (uint256));
        assembly ("memory-safe") {
            // We can skip all the checks performed by `abi.decode` because we know that this is the
...
abstract contract UniswapV4 is SettlerAbstract {
        ...
        returns (bool, bytes calldata)
    {
        ...
        key.hooks = IHooks_0.wrap(address(uint160(packed)));
    ...
    function unlockCallback(bytes calldata data) private returns (bytes memory) {
        ...
        while (data.length >= _HOP_DATA_LENGTH) {
            ...
            BalanceDelta_0 delta = IPoolManager(msg.sender).unsafeSwap(key, params, hookData);
            {
                (int256 settledSellAmount, int256 settledBuyAmount) =
                    zeroForOne.maybeSwap(delta.amount1(), delta.amount0());
